...
class Home extends Component {
  ...
  render() {
    ...
    for (let i = 0; i < 400; i++) { // 星星數量
      stars.push(<Entity geometry={{
        primitive: 'sphere',
        radius: Math.random() * .05 + .01
      }} material={{ color: '#fff' }} position={[(Math.random() * 80 - 40), (Math.random() * 300 - 150), (Math.random() * 200 - 100)]} />);
    }
      ...
      <figure className="pof full">
        ...
        <Scene vr-mode-ui={{ enabled: false }}>
          ...
          <Entity geometry={{
            primitive: 'sphere',
            radius: 3
          }} material={{ color: '#F5F3CE' }} position={[91, 106, -82]} />

          {/*火星*/}
          <Entity geometry={{
            primitive: 'sphere',
            radius: .8
          }} material={{ color: '#FFD0D0' }} position={[-46, 74, -135]} />
          {stars}

          {/*漸層背景*/}
          <Entity primitive="a-gradient-sky" material={{
            topColor: '0.00390625 0.09375 0.1796875',
            bottomColor: '0.0078125 0.12109375 0.23046875'
          }} />
        </Scene>
      </figure>
    );
